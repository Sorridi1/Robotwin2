...
from utils.rise_tcp import parse_rise_tcp_pose
...
class RealWorldDataset(Dataset):
    ...
    def __init__(
        ...
        for i in range(self.num_demos):
            ...
            demo_name = self.all_demos[i]          # e.g. "demonstration_72"
            demo_id = int(demo_name.split("_")[-1])  # 72
            #过滤
            # skip_ids = {102, 78, 79, 80, 82, 83, 86, 89, 90, 96, 97, 98, 99}
            # skip_ids = {102, 78, 79, 80, 82, 83, 86, 89, 90, 96, 97, 98, 99,100,101,81,94,95}

            # if demo_id in skip_ids:
            #     continue
            if demo_id==136 or demo_id==137:
                continue
            if demo_id==141 or demo_id==142:
                continue
            # clean_json_path = os.path.join(
            #     "/home/robot/RISE-fzc/clean_dataset",
            #     f"cleaned_dataset{demo_id}.json"
            # )
            # print("clean_json_path",clean_json_path)
            # with open(clean_json_path, "r") as f:
            #     clean_data = json.load(f)

            # indices = clean_data["meta"]["indices"]
            ...
            for cam_id in cam_ids:
                ...
                frame_ids = [
                    ...
                    if int(os.path.splitext(x)[0]) <= meta["finish_time"]
                            ...
                            # list & sort color files
                color_dir = os.path.join(cam_path, "color")
                color_files = sorted([
                    x for x in os.listdir(color_dir)
                    if x.endswith(".png")
                ])
                #过滤
                # print(len(indices), len(color_files))
                # frame_ids = []
                # for idx in indices:
                #     if 0 <= idx < len(color_files):
                #         frame_ids.append(
                #             int(os.path.splitext(color_files[idx])[0])
                #         )
                print(len(frame_ids))
    ...
    def __getitem__(self, index):
        ...
        for frame_id in action_frame_ids:
            tcp_path = os.path.join(tcp_dir, "{}.npy".format(frame_id))
            tcp = parse_rise_tcp_pose(np.load(tcp_path), source=tcp_path)
        ...
        #     traj.append(frame)
        # o3d.visualization.draw_geometries([pcd.voxel_down_sample(self.voxel_size), *traj])
